...
module Program =
    ...
    let withConsoleTrace (program: Program<'arg, 'model, 'msg, 'view>) =
        let inline toPlain o = o |> sprintf "%A"
        let traceInit (arg:'arg) =
            let initModel,cmd = program.init arg
            toPlain initModel |> sprintf "Initial state: %s" |> log
            initModel,cmd

        let traceUpdate msg model =
            toPlain msg |> sprintf "New message: %s" |> log
            let newModel,cmd = program.update msg model
            toPlain newModel |> sprintf "Updated state: %s" |> log
            newModel,cmd
